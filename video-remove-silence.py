...
parser.add_argument('--threshold-duration', type=float, default=0.6, help='threshold duration in seconds')
...
parser.add_argument('--linear', type=float, default=0.05, help='duration linear transform factor')
...
def report_progress(phase, current, total, message):
    ...
    print('__PROGRESS__ ' + json.dumps(payload, ensure_ascii=False), flush=True)
...
def find_silences(filename):
    ...
    with wave.open(filename) as wav:
        ...
        blend = 0
        detect_total = max(1, size)
        detect_report_step = max(1, detect_total // 80)
        detect_processed = 0
        ...
        def get_is_silence(blend):
            ...
            for index in range(half_blend_frames):
                ...
                blend += square
                detect_processed += 1
                if detect_processed % detect_report_step == 0 or detect_processed == detect_total:
                    report_progress('detect', detect_processed, detect_total, 'Sessizlikler tespit ediliyor...')
...
report_progress('extract', 0, 1, 'Sesler analiz ediliyor...')
extract_audio(args.path, audio_file.name)
report_progress('extract', 1, 1, 'Sesler analiz ediliyor...')
...
print('Finding gaps...')
report_progress('detect', 0, 1, 'Sessizlikler tespit ediliyor...')
silences, including_end = find_silences(audio_file.name)
report_progress('detect', 1, 1, 'Sessizlikler tespit ediliyor...')

total_duration = sum(( end-start for start, end in silences ))

if len(silences) == 0:
    report_progress('merge', 1, 1, 'Video işleniyor...')
    name, extension = os.path.splitext(args.path)
    out_file = '{}_result{}'.format(name, extension)
    shutil.copy2(args.path, out_file)
...
if args.recalculate_time_in_description:
    ...
    time_codes = {}
    for time_code, h, m, s in re.findall(r'((\d+):(\d\d):(\d\d))', description):
...
process_total = max(1, frames)
process_report_step = max(1, process_total // 180)
processed_input_frames = 0
report_progress('process', 0, process_total, 'Video yeniden kurgulanıyor...')
for start, end, is_silence in regions:
    start_frame = int(start * frame_rate)
    end_frame = frames if end is None else int(end * frame_rate)
    ...
    for index in range(start_frame, end_frame):
        ...
        if processed_input_frames % process_report_step == 0 or processed_input_frames >= process_total:
            report_progress('process', processed_input_frames, process_total, 'Video yeniden kurgulanıyor...')
